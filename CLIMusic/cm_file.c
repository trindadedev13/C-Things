...
struct cm_file_list_dir_data* cm_file_list_dir(char* path) {
  struct cm_file_list_dir_data* tld = malloc(sizeof(struct cm_file_list_dir_data));
  tld->dirs = malloc(sizeof(struct cm_file) * CM_FILE_LIST_DIR_INITIAL_CAPACITY);  // 100
  tld->len = 0;
